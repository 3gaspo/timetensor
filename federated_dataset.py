...
def run(cfg):
    ...
    for k in range(N):
        ...
        loaders_dict = get_train_loaders(node_data_dict[f"node_{k}"], batch_size, lags, horizon, by_date=True, subsets=cfg.fed.subsets[f"node{k}"], path=data_path + f"/node_{k}/")
