...
def run(cfg):
    ...
    df, context, datetimes = fetch_csv(data_path, dataset_name, context_cols=cfg.data.context_cols)
    ...
    stats_dict = get_dataset_stats(df, df_dict, lags, horizon, remove_cte, logger, data_path, "normal_stats.json")
    ...
    if clusters is not None:
        ...
        for i, df_dict in enumerate(df_dicts.values()):
            ...
            stats_dict = get_dataset_stats(cluster_df, df_dict, lags, horizon, remove_cte, logger, data_path+clusters+"stats/", f"node{i}_raw_stats.json")
    ...
    if replot:    
        ...
        plot_clustering(fourier_df, n_clusters, lags, horizon, "fourier_clusters", data_path, plot_dir, do_heterogeneity, logger, remove_cte)
        ...
        plot_clustering(gamma_df, n_clusters, lags, horizon, "gamma_clusters", data_path, plot_dir, do_heterogeneity, logger, remove_cte)
    ...
    for clusters in ["fourier_clusters/", "gamma_clusters/"]:
        ...
        for i, df_dict in enumerate(df_dicts.values()):
            stats_dict = get_dataset_stats(df, df_dict, lags, horizon, remove_cte, logger, data_path+clusters+"stats/", f"node{i}_raw_stats.json")
