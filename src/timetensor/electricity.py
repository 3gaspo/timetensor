...
def fetch_csv_data(path, years=None, drop=True):
...
def build_dataset(data_path, raw_format="csv", output_format="torch", years=None, hourly=False, drop=True):
    ...
    if raw_format == "txt":
        values_pt, datetimes = fetch_txt_data(data_path, years, hourly)
    elif raw_format == "csv":
        #load csv
        values_df, _, datetimes = fetch_csv_data(data_path, years, drop=drop)
        #tensors
